...
class V27Engine:
    ...
    class CandidateSheetGenerator:
        ...
        def generate_candidates(self, remaining_total: int) -> List[int]:
            ...
            for i in range(max(1, base_sheets - 3), base_sheets + 4):
                if i > 0 and i <= self.max_sheets:
                    candidates.append(i)
            
            if 1 not in candidates:
                candidates.append(1)
            if base_sheets not in candidates:
    ...
    # ================================================================
    # 4. MULTI PLATE SIMULATOR
    # ================================================================
    class MultiPlateSimulator:
        def __init__(self, max_plates: int, capacity: int):
            self.max_plates = max_plates
            self.capacity = capacity
            self.plate_builder = V27Engine.PlateBuilder(capacity)
        
        def simulate_round(self, remaining: dict, sheets: int) -> Tuple[List[dict], dict]:
            plates = []
            ...
            for plate_num in range(self.max_plates):
                ...
                for tag, ups in layout.items():
                    current_remaining[tag] = max(0, current_remaining.get(tag, 0) - (ups * sheets))
            ...
            return plates, current_remaining
    ...
    class SearchEngine:
        ...
        def search(self, demand: dict) -> Tuple[list, dict]:
            ...
            for strategy in strategies:
                for _ in range(self.iterations // len(strategies)):
        ...
        def _simulate_with_strategy(self, demand: dict, strategy: str) -> Tuple[list, dict]:
            ...
            while any(v > 0 for v in remaining.values()) and round_num < 10:
                ...
                if strategy == "greedy":
                    sheets = math.ceil(remaining_total / capacity_per_round)
                elif strategy == "balanced":
                    sheets = max(1, math.floor(remaining_total / capacity_per_round))
                elif strategy == "proportional":
                    sheets = max(1, int(remaining_total / capacity_per_round))
                else:
                    base = math.ceil(remaining_total / capacity_per_round)
                    sheets = max(1, base + random.randint(-2, 2))
                
                sheets = min(sheets, 100)
                
                simulator = V27Engine.MultiPlateSimulator(self.max_plates, self.capacity)
                plates, new_remaining = simulator.simulate_round(remaining, sheets)
                
                all_plates.extend(plates)
                remaining = new_remaining
                round_num += 1
            
            all_plates = ensure_demand_met(all_plates, demand)
            return all_plates, {"strategy": strategy, "rounds": round_num}
...
def v27_optimizer(demand: dict, capacity: int, max_plates: int, iterations: int = 50) -> list:
    """V27 Engine - Complete Optimization"""
